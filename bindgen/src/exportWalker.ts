...
export abstract class ExportsWalker {
  ...
  workset: Set<ClassPrototype> = new Set();
  ...
  visitElement(name: string, element: Element): void {
    ...
    var seen = this.seen;
    if (seen.has(element)) {
      if (<string>seen.get(element) != name) {
  ...
  private visitClassInstances(name: string, element: ClassPrototype): void {
    ...
    if (instances) {
      for (let instance of instances.values()) {
        if (instance.is(CommonFlags.COMPILED) || this.workset.has(element)) this.visitClass(name, <Class>instance);
      }
    }
  }
  ...
  visitClass(name: string, element: Class): void {
    ...
    if (instanceMembers) {
      for (let member of instanceMembers.values()) {
        if (base && base.members && base.members.has(member.name)) { continue; }
        if (member.kind == ElementKind.FIELD) {
          this.visitField(member.name, <Field>member);
  ...
  typeToString(type: Type): string {
    ...
    if (type.is(TypeFlags.REFERENCE)) {
      if (type.classReference!.name == "String"){
        ...
        this.workset.add(<ClassPrototype>this.program.elementsByName.get(typeStr)!);
      }
    }
